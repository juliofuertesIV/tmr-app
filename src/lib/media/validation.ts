...
export const mediaPayloadIsValidLength = ({ bytes } : { bytes: ArrayBuffer }) => {
    ...
    console.log({ bytes: bytes.byteLength, byteLimit })
...
export const validateMediaFormData = ({ formData } : { formData: FormData }) => {

    const fields = ['width', 'height', 'role', 'width', 'alt', 'height']

    fields.forEach(field => {
        if (formData.get(field) == undefined) 
            throw new Error('Falta un campo en la FormData: ' + field)
    })
